...
@SideEffectFree
@Tags({"uniswapv2", "sync"})
@CapabilityDescription("uniswapv2实时数据同步抓取")
public class UniswapV2RealTimeProcessor extends AbstractProcessor {
    ...
    public UniswapV2RealTimeProcessor() {
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        String currentBlockNumber = jedis.get("uniSwapV2CurrentRealTimeBlockNumber");
        if (currentBlockNumber==null){
            currentBlockNumber = blockNumber;
            jedis.set("uniSwapV2CurrentRealTimeBlockNumber",currentBlockNumber);
        }
        ...
        // validate data
        boolean isBlocked = isBlocked(post);
        if (isBlocked) {
            ...
            jedis.set("uniSwapV2CurrentRealTimeBlockNumber",String.valueOf(current));
            session.transfer(flowFile,REL_SUCCESS);
        } else {
            String retryTimes = jedis.get("retry"+currentBlockNumber);
            if (retryTimes!=null && Integer.parseInt(retryTimes)>3){
                Long current = Long.parseLong(currentBlockNumber) + 1;
                jedis.set("uniSwapV2CurrentRealTimeBlockNumber",String.valueOf(current));
            } else {
                try {
                ...
                // set retry
                if (retryTimes==null){
                    jedis.set("retry"+currentBlockNumber,"0");
                } else {
                    jedis.set("retry"+currentBlockNumber,String.valueOf(Integer.parseInt(retryTimes)+1));
                }
            }
            flowFile = session.write(flowFile, new StreamCallback() {
                @Override
    ...
    private boolean isBlocked(String post) {
        ...
        if (jsonObject!=null && !jsonObject.isEmpty()){
            if (jsonObject.containsKey("data")){
                ...
                System.out.println("jsonObject.containsKey(data)==boolean:"+jsonObject.toJSONString());
            }
        }else {
            System.out.println("jsonObject.isEmpty()==true:"+post);
        }
