...
@SideEffectFree
@Tags({"bsc uniswapv2", "swaps tokens pairs factory"})
@CapabilityDescription("bsc uniswapv2 token pair swap 数据同步抓取")
public class BscUniswapV2HistoryProcessor extends AbstractProcessor {
    ...
    public BscUniswapV2HistoryProcessor() {
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        if (!isFinish){
            ...
            jedis.set(redisCurrentSkipNumberKeyName,"0");
    ...
    private boolean isFinished(String post) {
        ...
        if (jsonObject!=null && !jsonObject.isEmpty()){
            if (jsonObject.containsKey("data")){
                ...
                if (data!=null){
                    JSONArray swaps = data.getJSONArray("swaps");
                    if (swaps==null){
                        isBlocked = true;
                    } else {
                        if (swaps.isEmpty()){
