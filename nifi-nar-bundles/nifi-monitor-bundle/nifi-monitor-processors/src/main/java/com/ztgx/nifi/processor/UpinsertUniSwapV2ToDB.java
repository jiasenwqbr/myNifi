...
public class UpinsertUniSwapV2ToDB  extends AbstractProcessor {
            ...
            for (int i = 0; i < transactions.size(); i++) {
                ...
                if (!swaps.isEmpty()) {
                    for (int j = 0; j < swaps.size(); j++) {
                        ...
                        if (!isExist(context,"select id from uniswap_v2_transaction_swap where id='"+swap.getString("id")+"'")){
                            insertTranactionSwap(swap,context,id);
                        }
    ...
    private  void insertTranactionSwap(JSONObject swap , ProcessContext context,String transId ){
    ...
    private void upInsertTranactions(JSONArray transactions, ProcessContext context)  {
        for (int i = 0; i < transactions.size(); i++){
            JSONObject transaction = transactions.getJSONObject(i);
            if (!isExist( context,"select id from uniswap_v2_transaction where id = '"+transaction.getString("id")+"'")){
                insertTranactions(transaction,context);
            }
        }
    }

    public void insertTranactions(JSONObject transaction, ProcessContext context)  {
        Connection conn = null;
        PreparedStatement stmt = null;
        Statement stmts = null;
        ResultSet rs = null;
    ...
    private boolean isExist(ProcessContext context,String sql){
        boolean exist = false;
        Connection conn = null;
        PreparedStatement stmt = null;
        Statement stmts = null;
